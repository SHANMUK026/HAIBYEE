...
const sendOTPViaAPI = async (phone, otp) => {
    try {
        // Ensure phone number has '91' prefix if it's a 10-digit number
        const formattedPhone = phone.length === 10 ? `91${phone}` : phone;
        const response = await fetch(process.env.MSG91_OTP_URL || "https://control.msg91.com/api/v5/oneapi/api/flow/otp-login/run", {
            method: "POST",
            headers: {
